...
function derive(){
  ...
  const Tann=T.reduce((a,b)=>a+b,0)/12, Pann=P.reduce((a,b)=>a+b,0);
...
function koppen(){
  ...
  const su=summerIdx(), wi=[...Array(12).keys()].filter(i=>!su.includes(i));
...
function svgMarkup(forExport){
  ...
  for(let i=0;i<12;i++){
    ...
    o.push(`<text x="${xc(i).toFixed(1)}" y="${PB+30}" text-anchor="middle" font-family="${F}" font-size="16" font-weight="${on?700:400}" fill="${on?"#1e242a":"#66737c"}">${i+1}</text>`);
  }
...
function draw(){
  ...
  document.getElementById("mLabel").textContent=MONTHS[state.sel];
...
function buildTable(){
  const cells=n=>MONTHS.map((m,i)=>`<td><input type="number" inputmode="decimal" data-k="${n}" data-i="${i}" step="${n==='T'?0.1:1}" value="${n==='T'?state.T[i].toFixed(1):Math.round(state.P[i])}" aria-label="${m}の${n==='T'?'気温':'降水量'}"></td>`).join("");
  document.getElementById("tblwrap").innerHTML=
    `<table class="grid"><thead><tr><th class="rh"></th>${MONTHS.map(m=>`<th>${m}</th>`).join("")}</tr></thead>
...
function load(){
  try{ const s=JSON.parse(localStorage.getItem("climograph-v1")||"null");
    if(s&&Array.isArray(s.T)&&s.T.length===12&&Array.isArray(s.P)&&s.P.length===12) Object.assign(state,s,{sel:clamp(s.sel|0,0,11)});
  }catch(e){}
}
...
wrap.addEventListener("pointerdown",e=>{
  ...
  const i=clamp(Math.floor((p.x-PL)/CW),0,11);
  let kind=state.mode;
  if(kind==="auto"){
...
$("mPrev").addEventListener("click",()=>{state.sel=(state.sel+11)%12;render();});
$("mNext").addEventListener("click",()=>{state.sel=(state.sel+1)%12;render();});
...
$("blank").addEventListener("click",()=>{ push(); state.T=Array(12).fill(0); state.P=Array(12).fill(0); state.name="　"; syncInputs(); buildTable(); render(); });
